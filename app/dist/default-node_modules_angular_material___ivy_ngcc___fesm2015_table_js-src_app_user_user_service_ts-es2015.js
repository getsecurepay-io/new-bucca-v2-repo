...
CdkTable.ctorParameters = () => [
    ...
    { type: undefined, decorators: [{ type: _angular_core__WEBPACK_IMPORTED_MODULE_0__.Optional }, { type: _angular_core__WEBPACK_IMPORTED_MODULE_0__.Inject, args: [_angular_cdk_collections__WEBPACK_IMPORTED_MODULE_1__._VIEW_REPEATER_STRATEGY,] }] },
    { type: _CoalescedStyleScheduler, decorators: [{ type: _angular_core__WEBPACK_IMPORTED_MODULE_0__.Optional }, { type: _angular_core__WEBPACK_IMPORTED_MODULE_0__.Inject, args: [_COALESCED_STYLE_SCHEDULER,] }] },
    ...
    { type: _angular_cdk_scrolling__WEBPACK_IMPORTED_MODULE_13__.ViewportRuler, decorators: [{ type: _angular_core__WEBPACK_IMPORTED_MODULE_0__.Optional }] }
];
...
(function () { (typeof ngDevMode === "undefined" || ngDevMode) && _angular_core__WEBPACK_IMPORTED_MODULE_0__["ɵsetClassMetadata"](CdkTable, [{
        ...
        args: [{
            ...
            }] }, { type: _angular_cdk_scrolling__WEBPACK_IMPORTED_MODULE_13__.ViewportRuler, decorators: [{
                type: _angular_core__WEBPACK_IMPORTED_MODULE_0__.Optional
            }] }]; }, { trackBy: [{
            type: _angular_core__WEBPACK_IMPORTED_MODULE_0__.Input
        }], dataSource: [{
...
class UserService {
    ...
    getCompanyList(pageSize = 10, pageIndex = 1) {
        ...
        this.http
            .get(`${this.apiUrl}auth/companys/${query}`)
            .subscribe((res) => {
    ...
    getDeviceList(pageSize = 10, pageIndex = 1) {
        const query = `?limit=${pageSize}&offset=${pageIndex}`;
        this.http
            .get(`${this.apiUrl}auth/devices/${query}`)
            .subscribe((res) => {
            this.deviceList = res;
            this.devicesListener.next(Object.assign({}, this.deviceList));
        }, (err) => { });
    }
    getDeviceListUpdate() {
    ...
    createDevice(formData, dialogRef) {
        ...
        this.http
            .post(`${this.apiUrl}auth/devices/`, Object.assign({}, formData))
            .pipe((0,rxjs_operators__WEBPACK_IMPORTED_MODULE_5__.shareReplay)())
            .subscribe((response) => {
            this.isLoadingListener.next(false);
            this.notificationService.success(`Success`, `Device onboard successful`);
            dialogRef.close();
        }, (err) => {
            console.log(err);
            const errorMessage = err.error.message || err.error.details || err.error.error || `device code: ${err.error.device_code[0]}` || `name: ${err.error.name[0]}` || `device id: ${err.error.device_id[0]}` || `${err.error.is_active[0]}` || err.error.company[0] || err.error.location[0];
            // this.notificationService.danger(`Error`, errorMessage);
            this.isLoadingListener.next(false);
            this.errorService.handleError(errorMessage);
        });
    ...
    getCompanyAdminList(pageSize = 10, pageIndex = 1, company) {
        ...
        this.http
            .get(`${this.apiUrl}auth/cmp-adm/${query}`)
            .subscribe((res) => {
    ...
    getAccountantList(pageSize = 10, pageIndex = 1, company) {
        const query = `?limit=${pageSize}&offset=${pageIndex}&${company ? "&company=" + company : ""}`;
        this.http
            .get(`${this.apiUrl}auth/cmp-act/${query}`)
            .subscribe((res) => {
    ...
    getStaffList(pageSize = 10, pageIndex = 0) {
        const query = `?limit=${pageSize}&offset=${pageIndex}`;
        this.http
            .get(`${this.apiUrl}auth/staffs/${query}`)
            .subscribe((res) => {
            // console.log(res)
            this.staffs = res;
    ...
    getStaffLevels() {
        this.http
            .get(`${this.apiUrl}auth/staff-level/`)
            .subscribe((res) => {
            console.log(res);
    ...
    getDepartments(pageSize = 10, pageIndex = 1) {
        const query = `?limit=${pageSize}&offset=${pageIndex}`;
        this.http
            .get(`${this.apiUrl}auth/departments/${query}`)
            .subscribe((res) => {
    ...
    getVendors(pageSize = 10, pageIndex = 1, company = null) {
        const query = `?limit=${pageSize}&offset=${pageIndex}&${company ? "&company=" + company : ""}`;
        this.http
            .get(`${this.apiUrl}auth/vendor/${query}`)
            .subscribe((res) => {
            this.vendors = res;
    ...
    getVendorStaffs(pageSize = 10, pageIndex = 1) {
        const query = `?limit=${pageSize}&offset=${pageIndex}`;
        this.http
            .get(`${this.apiUrl}auth/vendor-staff/${query}`)
            .subscribe((res) => {
...
//# sourceMappingURL=default-node_modules_angular_material___ivy_ngcc___fesm2015_table_js-src_app_user_user_service_ts-es2015.js.map